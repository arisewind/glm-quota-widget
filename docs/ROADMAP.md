...
- 文档版本：v1.1
- 制定日期：2026-07-18
- 状态：**v1.1 已完成**（2026-07-18，编译 + aapt2 静态验证通过，APK 13.1MB，待真机回归）；v2.0 待启动
...
| 版本 | 范围 | 验收 |
|---|---|---|
| **v1.1（体验补齐）✅** | 需求 5（服务商名）+ 设置页骨架 + Key 加密存储 + 前台刷新 + 需求 1（开屏） | 编译 + aapt2 静态验证通过（APK 13.1MB）；真机回归待做。单 GLM 下体验完整、Key 已加密 |
...
4. 🚧 **v2.0 阶段 A**——架构改造（[ADR-0002](adr/0002-multi-provider-normalization.md)：`ServiceProvider` 注册表 + `UsageSnapshot` + 多账户），GLM 接入新架构验证。
5. 🚧 **v2.0 阶段 B**——Kimi 接入（[ADR-0003](adr/0003-kimi-usage-direct.md)，go/no-go 关卡）。
