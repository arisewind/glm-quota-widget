...
- 文档版本：v1.1
- 制定日期：2026-07-18
- 状态：**v1.1 已完成**；**v2.0 阶段 A/B/C 已完成**（架构重构 + Kimi + MiniMax 接入，2026-07-18，编译 + 10 单测过，commit f9848c2）；GLM 真机回归 + 火山/ZenMux 待做
...
7. 🚧 **待做**——GLM 真机回归（验证 v1.x 单 Key→Account 迁移 + 新架构不破坏 v1.1）；阶段 D（多账户 widget）；火山方舟/ZenMux（v2.1+）。
