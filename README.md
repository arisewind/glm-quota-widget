...
**当前状态:Android 版 v1.0 已交付,真机验证通过(2026-07-18)。** 后续规划见 [ROADMAP.md](docs/ROADMAP.md)。
...
- **Android 工程**:`C:\Users\11978\Desktop\glmquota`(AGP 9.3 / Kotlin 2.2.10 / Jetpack Compose / AppWidget / minSdk 26)
- **核心层**:TypeScript 平台无关核心(`src/`,32 单测全过)已移植为 Kotlin(`domain/` + `services/`)
- **产物**:`app-debug.apk`(16.9MB),真机(OCE-AN50 / HarmonyOS 4.2)端到端验证:配置 → 拉取真实用量 → 桌面 widget 显示数据,全链路通
...
Android 工程:在 `glmquota/` 用 `gradlew assembleDebug` 编译(需 JDK 21 + 阿里云镜像,详见 [ROADMAP.md](docs/ROADMAP.md) 环境说明)。
...
- 🚧 **v1.1**:服务商名标题 / 账户管理骨架 / Key 加密 / 前台刷新 / 开屏页
